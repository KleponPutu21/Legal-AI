<script setup lang="ts">
import { onMounted } from 'vue'
import { useRoute } from 'vue-router'
import { Button } from '@/transcript/components/ui/button'
import {
  Card,
  CardContent,
  CardDescription,
  CardHeader,
  CardTitle,
} from '@/transcript/components/ui/card'
import {
  Field,
  FieldDescription,
  FieldGroup,
  FieldLabel,
} from '@/transcript/components/ui/field'
import {
  InputOTP,
  InputOTPGroup,
  InputOTPSlot,
} from '@/transcript/components/ui/input-otp'
import { useOTPVerification } from '@/features/auth/composables'

// Auth logic
const route = useRoute()
const { email, otpCode, isLoading, error, initializeEmail, handleVerifyOTP } = useOTPVerification()

onMounted(() => {
  initializeEmail(route.query.email as string)
})
</script>

<template>
  <Card>
    <CardHeader class="text-center">
      <CardTitle class="text-xl">
        Enter verification code
      </CardTitle>
      <CardDescription>We sent a 6-digit code to {{ email || 'your email' }}.</CardDescription>
    </CardHeader>
    <CardContent>
      <form @submit.prevent="handleVerifyOTP">
        <FieldGroup>
          <!-- Error message -->
          <div v-if="error" class="bg-destructive/15 text-destructive text-sm p-3 rounded-md mb-4">
            {{ error }}
          </div>
          
          <Field>
            <FieldLabel for="otp" class="sr-only">
              Verification code
            </FieldLabel>
            <InputOTP id="otp" v-model="otpCode" :maxlength="6" required>
              <InputOTPGroup class="gap-2.5 *:data-[slot=input-otp-slot]:rounded-md *:data-[slot=input-otp-slot]:border">
                <InputOTPSlot :index="0" />
                <InputOTPSlot :index="1" />
                <InputOTPSlot :index="2" />
                <InputOTPSlot :index="3" />
                <InputOTPSlot :index="4" />
                <InputOTPSlot :index="5" />
              </InputOTPGroup>
            </InputOTP>
            <FieldDescription class="text-center">
              Enter the 6-digit code sent to your email.
            </FieldDescription>
          </Field>
          <Button type="submit" :disabled="isLoading || otpCode.length !== 6">
            {{ isLoading ? 'Verifying...' : 'Verify' }}
          </Button>
          <FieldDescription class="text-center">
            Didn't receive the code? <a href="#">Resend</a>
          </FieldDescription>
        </FieldGroup>
      </form>
    </CardContent>
  </Card>
</template>
